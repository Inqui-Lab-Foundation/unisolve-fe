...
const UsersPage = () => {
  ...
  const currentLanguage = languageOptions.find(l => l.code === currentLanguageCode)
  const cardProps = {
    primary: true,
    label: "ImageCardComp",
    imgUrl: Nature,
    ...
    <div className="userPage">
      ...
      <DropdownButton id="dropdown-basic-button" title={<BsGlobe2 />}>
        {languageOptions.map( (item) => {
          return(
            <Dropdown.Item href="#/action-1" onClick={() => i18next.changeLanguage(item.code)}>
              <span className={`flag-icon flag-icon-${item.country_code}`}> {item.name}</span>
            </Dropdown.Item>
            )
